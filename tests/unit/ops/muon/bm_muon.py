...
def main_new(run, model):
    ...
    # optimizer2 = torch.optim.SGD(filter_params, lr=0.24, momentum=0.6, nesterov=True)
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            engine.step()
            step += 1
            # print(f"step is {step}, whiten_bias_train_steps is {whiten_bias_train_steps}")
...
class WrapModel(torch.nn.Module):
    ...
    def reset(self,):
        self.cifarnet.reset()
    def init_whiten(self, inputs):
        return self.cifarnet.init_whiten(inputs)
